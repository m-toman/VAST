...
def create_train_dataloaders(args):
    ...
    for d_cfg in data_cfg:
        ...
        loader = build_dataloader(
            ...
            collate_fn,
            True,
            batch_size // args.run_cfg.gradient_accumulation_steps,
            n_workers,
            worker_init_fn,
            use_sampler,
        )
    ...
    for name, (loader, ratio) in dataloaders_dict.items():
        ...
        LOGGER.info(
            f" loader {name} , ratio {ratio} , bs_pergpu {loader.batch_size}, n_workers {loader.num_workers}"
        )
    ...
    args.run_cfg.valid_steps = (
        args.run_cfg.num_train_steps // args.run_cfg.valid_freq - 1
    )
