...
class VAST(MMGeneralModule):
    ...
    def __init__(self, config):
        ...
        self.contra_head_vs = nn.Linear(
            self.vision_dim + self.multimodal_dim, contra_dim
        )
        self.contra_head_vas = nn.Linear(
            self.vision_dim + self.audio_dim + self.multimodal_dim, contra_dim
        )
        ...
        self.vision_frame_embedding = nn.Parameter(
            0.02
            * torch.randn(1, self.config.max_vision_sample_num, self.multimodal_dim)
        )
        self.audio_frame_embedding = nn.Parameter(
            0.02 * torch.randn(1, self.config.max_audio_sample_num, self.multimodal_dim)
        )
        self.hidden_trans_vision_multimodal = nn.Sequential(
            nn.Linear(self.vision_dim, self.multimodal_dim),
            LayerNorm(self.multimodal_dim, eps=1e-12),
        )
        self.hidden_trans_audio_multimodal = nn.Sequential(
            nn.Linear(self.audio_dim, self.multimodal_dim),
            LayerNorm(self.multimodal_dim, eps=1e-12),
        )
        self.hidden_trans_subtitle_multimodal = nn.Sequential(
            nn.Linear(self.multimodal_dim, self.multimodal_dim),
            LayerNorm(self.multimodal_dim, eps=1e-12),
        )
        self.vision_type_embeddings = nn.Parameter(
            0.02 * torch.randn(1, 1, self.multimodal_dim)
        )
        self.audio_type_embeddings = nn.Parameter(
            0.02 * torch.randn(1, 1, self.multimodal_dim)
        )
        self.subtitle_type_embeddings = nn.Parameter(
            0.02 * torch.randn(1, 1, self.multimodal_dim)
        )
    ...
    def construct_multimodal_encoder(self):
        ...
        self.multimodal_encoder = BertForMaskedLM.from_pretrained(
            "./pretrained_weights/bert/bert-base-uncased", config=bertconfig
        )
        self.multimodal_dim = 768

        if self.config.checkpointing:
            self.multimodal_encoder._set_gradient_checkpointing(
                self.multimodal_encoder.bert.encoder, True
            )

        from transformers import BertTokenizer

        self.multimodal_encoder.tokenizer = BertTokenizer.from_pretrained(
            "./pretrained_weights/bert/bert-base-uncased"
        )
        self.multimodal_encoder.tokenizer.bos_token_id = (
            self.multimodal_encoder.tokenizer.convert_tokens_to_ids(["[CLS]"])[0]
        )
        self.multimodal_encoder.tokenizer.eos_token_id = (
            self.multimodal_encoder.tokenizer.convert_tokens_to_ids(["[SEP]"])[0]
        )
        self.multimodal_encoder.tokenizer.pad_token_id = (
            self.multimodal_encoder.tokenizer.convert_tokens_to_ids(["[PAD]"])[0]
        )
        self.multimodal_encoder.tokenizer.mask_token_id = (
            self.multimodal_encoder.tokenizer.convert_tokens_to_ids(["[MASK]"])[0]
        )

        self.text_masker = TokenMasker(
            mask_token=self.multimodal_encoder.tokenizer.mask_token_id,
            range_start=106,
            range_end=30522,
        )
    ...
    def batch_get(self, batch, key):
        if key in batch:
            ...
            condition_feats_s = self.get_multimodal_forward_input_subtitle(
                subtitle_output
            )
            ...
            condition_feats_va = torch.cat(
                (condition_feats_v, condition_feats_a), dim=1
            )
            ...
            condition_feats_vs = torch.cat(
                (condition_feats_v, condition_feats_s), dim=1
            )
            ...
            condition_feats_vas = torch.cat(
                (condition_feats_v, condition_feats_a, condition_feats_s), dim=1
            )
            ...
            subtitle_output_pooled = self.pool_text_for_contra(subtitle_output)
            feat_vas = torch.cat(
                (vision_output_pooled, audio_output_pooled, subtitle_output_pooled),
                dim=1,
            )
            ...
            input_ids = caption_tokens.input_ids
            attention_mask = caption_tokens.attention_mask
            caption_tokens = self.multimodal_encoder.bert(
                input_ids=input_ids, attention_mask=attention_mask
            ).last_hidden_state
    ...
    def forward(self, batch, task, compute_loss=True):
        ...
        if (
            "vision_captions" in batch
            or "audio_captions" in batch
            or "omni_captions" in batch
        ):
            assert compute_loss
    ...
    def compute_slice_scores(
        self, slice_multimodal_vision_input, slice_input_ids, slice_attention_mask
    ):
        ...
        if compute_loss:
            ...
            for task in subtasks:
                ...
                attention_mask_1 = torch.cat(
                    (attention_mask, attention_mask, text_atts_neg), dim=0
                )

                condition_feats = torch.cat(
                    (condition_feats, condition_feats_neg, condition_feats), dim=0
                )
    ...
    def forward_cap(self, batch, task, compute_loss=True):
        ...
        if compute_loss:
            ...
            attention_mask[:, :seq_len, :seq_len] = torch.tril(
            ...
            for task in subtasks:
                ...
                init_input_ids = (
                    ...
                    .long()
                    .cuda()
                    .fill_(self.multimodal_encoder.tokenizer.bos_token_id)
                )
                init_attention_mask = init_input_ids.new_ones(batch_size, 1, 1)

                if self.config.captioner_mode:
                    condition_feats = (
                        condition_feats.unsqueeze(1)
                        .expand(-1, self.config.generate_nums, -1, -1)
                        .reshape(-1, *condition_feats.shape[1:])
                    )
                    outputs = self.multimodal_encoder.generate(
                ...
                captions = self.multimodal_encoder.tokenizer.batch_decode(
                    outputs_newgen, skip_special_tokens=True
                )
    ...
    def forward_qa(self, batch, task, compute_loss=True):
        ...
        question_tokens = self.multimodal_encoder.tokenizer(
            raw_questions,
            padding="max_length",
            truncation=True,
            max_length=self.max_caption_len,
        ...
        question_tokens_ids, question_tokens_mask = (
        ...
        if compute_loss:
            ...
            answer_tokens = self.multimodal_encoder.tokenizer(
                raw_answers,
                padding="max_length",
                truncation=True,
                max_length=10,
            ...
            answer_tokens_ids, answer_tokens_mask = (
            ...
            attention_mask = torch.cat(
                (question_tokens_mask, answer_tokens_mask), dim=1
            )
            ...
            seq_len = attention_mask.shape[1]
            attention_mask = attention_mask.unsqueeze(1).expand(-1, seq_len, -1).clone()
            attention_mask[:, question_len:seq_len, question_len:seq_len] = torch.tril(
                attention_mask[:, question_len:seq_len, question_len:seq_len]
            )
            ...
            init_input_ids = torch.cat(
                (question_tokens["input_ids"], init_input_ids), dim=1
            )
            ...
            attention_mask = (
                question_tokens["attention_mask"]
                .unsqueeze(1)
                .expand(-1, question_len, -1)
                .clone()
            )
            init_attention_mask = self.multimodal_encoder.update_attention_mask(
                attention_mask
            )
            ...
            for task in subtasks:
                ...
                for i in range(condition_feats.shape[0]):
                    condition_feats_expand.append(
                        condition_feats[i : i + 1].expand(num_questions[i], -1, -1)
                    )
                ...
                answers = self.multimodal_encoder.tokenizer.batch_decode(
                    outputs_newgen, skip_special_tokens=True
                )
                print(answers)
        ...
        if compute_loss:
            ...
            for task in subtasks:
                ...
                if task == "tv":
                    ...
                    caption_tokens = self.batch_get(
                        batch, "omni_caption_tokens", txt_len=self.max_omni_caption_len
                    )
